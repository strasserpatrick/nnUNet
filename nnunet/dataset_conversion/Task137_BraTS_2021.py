...
def preprocess_brats21(task_id=137, task_name="BraTS2021", excluded_case_ids=None):
    ...
    if excluded_case_ids is None:
        ...
        print(f"excluding {len(excluded_case_ids)} cases")
    ...
    patients_folder_list = [p for p in subdirs(cur, join=False) if p not in excluded_case_ids]
    assert len(patients_folder_list) == len(subdirs(cur, join=False)) - len(excluded_case_ids)
    for p in tqdm(patients_folder_list):
        patdir = join(cur, p)
        patient_name = p
...
if __name__ == "__main__":
    ...
    excluded_case_ids = mapping_df[~mapping_df["BraTS2021"].isna() & ~mapping_df["BraTS2019"].isna()][
        "BraTS2021"].tolist()
    preprocess_brats21(300, excluded_case_ids=excluded_case_ids)
