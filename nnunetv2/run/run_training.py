...
def maybe_load_checkpoint(
    ...
    if continue_training and (
        pretrained_weights_file is not None or encoder_weights_file is not None
    ):
        raise RuntimeError(
    ...
    if continue_training:
        ...
        if pretrained_weights_file and encoder_weights_file:
            raise RuntimeError(
                "Cannot initialize both encoder weights and full network weights"
            )
        if pretrained_weights_file or encoder_weights_file:
            ...
            load_pretrained_weights(
                nnunet_trainer.network,
                weights,
                encoder_only=(pretrained_weights_file is None),
                verbose=True,
            )
        expected_checkpoint_file = None
...
def run_ddp(
    ...
    **kwargs,
    ...
    nnunet_trainer = get_trainer_from_args(
        dataset_name_or_id, configuration, fold, tr, p, use_compressed, **kwargs
    )
    ...
    maybe_load_checkpoint(nnunet_trainer, c, val, pretrained_weights, encoder_weights)
...
def run_training_entry():
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="[OPTIONAL] path to nnU-Net checkpoint file to be used for loading pretrained encoder weights.",
    )
    parser.add_argument(
        "-num_gpus",
        type=int,
